class Login {
    ...
    validateonSubmit() {
        const self = this; // setup calls to the "this" values of the class described in the constructor
        ...
        this.form.addEventListener("submit", (event) => {
            ...
            if (error === 0) {
                //TODO: аутентификация
                const userId = 1; // TOSO: заменить на авторизованного пользователя
    ...
    validateFields(field) {
        ...
        if (field.value.trim() === "") {
            ...
            this.setStatus(
                field,
                `${field.previousElementSibling.innerText} заполните`,
                "error"
            );
            return false;
        } else {
            ...
            if (field.type == "password") {
                ...
                if (field.value.length < 8) {
                    ...
                    this.setStatus(
                        field,
                        `${field.previousElementSibling.innerText} мин 6 знаков`,
                        "error"
                    );
                    return false;
                } else {
